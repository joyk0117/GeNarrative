...
Semantic Interface Structure（SIS）は、**「作品やシーンの意味情報」を取り出し、生成・検索・再利用の“ハブ”にする意味スキーマ**です。  
...
SIS はこの発想を 3D に限定せず、物語・画像・音声などマルチモーダル創作に拡張し、「意味」を編集可能な意味スキーマとして扱うことを目的とします。
...
SISは「モーダル間の連携のための“意味スキーマ”」という位置づけですが、同じ目的は別の設計でも達成できます。
...
- **概要**：画像や文章からキャプション/指示文を作り、そのまま各モーダル生成（文章/画像/音楽など）に流す。意味スキーマを固定せず、パイプで直結する方式。
...
- **概要**：画像や音声などを埋め込みベクトルに落とし、「近いもの」を検索したり条件付けに使ったりする方式（意味スキーマがベクトル）。
- **向いている**：大量資産の検索・推薦、類似性ベースの再利用。
- **弱点**：人間が編集しにくくブラックボックス寄り。検証や「この属性だけ変える」が難しい。

#### E) グラフ（知識グラフ/シーングラフ）
- **概要**：「登場人物Aが物体Bを持つ」「場所は森」など、ノード・エッジの関係構造として意味スキーマを持つ方式。
