...
## 🌟 概要 (Overview)
GeNarrative はローカル環境で動作するマルチモーダル創作支援アシスタントです。台本・イラスト・音声・BGM を組み合わせ、オフラインでオリジナルの物語体験を作れます。各機能は Docker で分離され、プライバシー・再現性・拡張性を重視しています。

- 入力例: 台本、子どもの手描きイラスト
- 出力例: ナレーション付きマルチメディア物語 (HTML / MP4)
...
- 生成物から SIS を再抽出して、検索や評価に利用することもできます。
...
事前に Docker Desktop をインストールしてください（GPU 利用時は WSL2 + NVIDIA 環境が必要）。
