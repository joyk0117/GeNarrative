...
> ※ 本リポジトリは実験用のプロジェクトであり、現在も開発・検証中です。仕様や挙動は予告なく変更される可能性があります。
...
## ✨ 主な機能
- **画像（またはテキスト）からのシーン生成**
  - 「画像 or テキスト → 画像・テキスト・音声・音楽」の一連の生成フロー
- **単一シーンを起点としたストーリー全体の自動生成**
- **生成したストーリーを絵本形式で出力**
  - HTML（単一ファイル）または MP4 動画としてエクスポート可能
...
- 外部のAIサービスとの連携
- 音楽および音声からのSIS生成
- インストール時間と容量の削減(uv, Dockerコンテナの最適化)
